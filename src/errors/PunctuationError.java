		...
		while (totalNums > 0 && errNum > 0) {
			...
			for (int para = 0; para < result.size(); para++) {
				if (num >= result.get(para).size()) {
					...
					if (String.valueOf(punctuationList[n]).equals(r.getValue())
							&& n < punctuationList.length - 1) {
						corpus_bf.get(para).replace(r.getLocation(),
								r.getLocation() + 1,
								String.valueOf(punctuationList[n + 1]));
						// System.out.println("ori:"+
						// r.getValue()+"Rep"+String.valueOf(punctuationList[n+1]));
						...
						corpus_bf.get(para).replace(r.getLocation(),
								r.getLocation() + 1,
								String.valueOf(punctuationList[n]));
						// System.out.println("ori:"+
						// r.getValue()+"Rep"+String.valueOf(punctuationList[n]));
						Sign sign = new Sign(para, r.getLocation(),
								r.getValue(),
								String.valueOf(punctuationList[n]));
						this.getSigns().add(sign);
